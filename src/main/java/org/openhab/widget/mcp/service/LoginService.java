...
public class LoginService {
	...
	public synchronized void login(Page p) {
		...
		if (!staticToken.isBlank()) {
			...
			try {
				p.evaluate("token => {" + "  localStorage.setItem('openhab.ui:token', token);"
		...
		try {
			...
			if (!"/auth".equals(BrowserService.pathOf(p.url()))) {
				...
				try {
					...
					p.waitForURL(url -> "/auth".equals(BrowserService.pathOf(url)),
			...
			p.waitForSelector("input[placeholder='User Name']", new Page.WaitForSelectorOptions().setTimeout(10000));
