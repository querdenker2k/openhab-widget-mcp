...
public class ItemResource {
  ...
  @Operation(
      ...
      description =
          "Returns all OpenHAB items. Optional ?filter= parameter for name-based filtering.")
  public Response listItems(
      @Parameter(description = "Case-insensitive name filter, e.g. charge") @QueryParam("filter")
          String filter) {
  ...
  @Operation(
      ...
      description = "Returns the current state of the item as plain text.")
  ...
  @Operation(
      summary = "Send command to item",
      description =
          "Sends a command to an OpenHAB item. Body is plain text: ON, OFF, 0-100, UP, DOWN, STOP, etc.")
  public Response sendCommand(
      @Parameter(description = "Item name, e.g. MySwitch") @PathParam("name") String name,
      String command) {
