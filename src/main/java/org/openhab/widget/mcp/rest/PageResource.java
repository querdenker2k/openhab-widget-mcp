...
public class PageResource {
  ...
  @Operation(
      summary = "Create or update a canvas page",
  ...
  public Response createOrUpdatePage(PageRequest request) {
    Log.infof(
        "REST createOrUpdatePage: uid=%s, label=%s, widgetUid=%s",
        request.uid(), request.label(), request.widgetUid());
    PageService.CreateOrUpdatePage result =
        pageService.createOrUpdatePage(
            request.uid(),
            request.label(),
            request.widgetUid(),
  ...
  @Path("/{uid}/testpage")
  @Operation(
      summary = "Create or update a test page that embeds this widget",
      description =
          "Convenient endpoint to spin up a persistent OpenHAB page (visible in the sidebar) "
  ...
  public Response createTestPage(
      @Parameter(description = "Widget UID to embed, e.g. RD_car_charging_widget") @PathParam("uid")
          String widgetUid,
      @Parameter(description = "Page label shown in the sidebar (default: widget UID)")
          @QueryParam("label")
          String label,
      @Parameter(description = "Page UID (default: widget UID)") @QueryParam("pageUid")
          String pageUid,
      @Parameter(
              description = "JSON object with widget props, e.g. {\"title\":\"Auto\"}. Default: {}")
          @QueryParam("propsJson")
          String propsJson) {
    ...
    Log.infof(
        "REST createTestPage: widgetUid=%s, pageUid=%s, label=%s, props=%s",
        widgetUid, resolvedPageUid, resolvedLabel, resolvedProps);
    PageService.CreateOrUpdatePage message =
        pageService.createOrUpdatePage(resolvedPageUid, resolvedLabel, widgetUid, resolvedProps);
  ...
  public record PageRequest(
      ...
      @Parameter(description = "JSON object with widget props, e.g. {\"title\": \"Auto\"}")
          String propsJson) {}
}
