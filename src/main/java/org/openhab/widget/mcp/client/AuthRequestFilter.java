...
import org.openhab.widget.mcp.service.LoginService;
 ...
 *   <li>OAuth2 access token extracted from {@link LoginService}'s logged-in browser session
...
public class AuthRequestFilter implements ClientRequestFilter {
    ...
    public void filter(ClientRequestContext ctx) {
        ...
        try {
            LoginService loginService = Arc.container().instance(LoginService.class).get();
            if (loginService == null) {
                return;
            }
            String token = loginService.getAccessToken();
            if (token.isBlank()) {
                token = loginService.ensureToken();
            }
            if (!token.isBlank()) {
