...
public interface OpenHabClient {
	...
	@Consumes(MediaType.APPLICATION_JSON)
	Response setItemMetadata(@PathParam("name") String name, @PathParam("namespace") String namespace, String body);

	@DELETE
	@Path("/items/{name}/metadata/{namespace}")
	Response deleteItemMetadata(@PathParam("name") String name, @PathParam("namespace") String namespace);

	// --- Persistence ---

	@PUT
	@Path("/persistence/items/{name}")
	Response addPersistenceData(@PathParam("name") String name, @QueryParam("time") String time,
