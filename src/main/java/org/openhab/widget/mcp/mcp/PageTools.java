...
public class PageTools {
  ...
  @Tool(
      description =
          "Take a screenshot of an OpenHAB page as it appears in the browser. "
  ...
  public PageService.CreateOrUpdatePage createTestPageForWidget(
      @ToolArg(description = "UID of the widget to embed, e.g. RD_car_charging_widget")
          String widgetUid,
      @ToolArg(required = false, defaultValue = "", description = "Optional page label")
          String label,
      @ToolArg(required = false, defaultValue = "", description = "Optional page UID")
          String pageUid,
      @ToolArg(
              required = false,
              defaultValue = "{}",
              description = "Optional widget props JSON, e.g. {\"title\":\"Auto\"}")
          String propsJson) {
  ...
  public PageService.CreateOrUpdatePage createPage(
    ...
    List<PageService.WidgetPlacement> placements =
        new ObjectMapper().readValue(placementsJson, new TypeReference<>() {});
