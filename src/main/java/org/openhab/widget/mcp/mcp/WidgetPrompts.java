...
public class WidgetPrompts {
	...
	@Prompt(name = "create_widget", title = "Create widget", description = "Creates a new OpenHAB Main UI widget from a description. "
	...
	PromptMessage createWidget(
			@PromptArg(name = "description", description = "What should the widget show or do? "
					+ "E.g. 'card showing the car's current charge state and range'") String description,
			@PromptArg(name = "size_hint", required = false, description = "Optional: desired size class (small/medium/large/wide/full)") String sizeHint,
			@PromptArg(name = "color_hint", required = false, description = "Optional: color scheme (inherit/accent/semantic/brand), "
					+ "with hex codes if applicable") String colorHint,
			@PromptArg(name = "design_reference", required = false, description = "Optional: another widget which should be used as reference for styling") String design_reference) {

		String body = CREATE_WIDGET_TEMPLATE.replace("{description}", nullSafe(description))
				.replace("{size_hint}", nullSafe(sizeHint)).replace("{color_hint}", nullSafe(colorHint))
				.replace("{design_reference}", nullSafe(design_reference));

		return PromptMessage.withUserRole(new TextContent(body));
	}

	@Prompt(name = "refine_widget", title = "Refine widget", description = "Refines an existing widget. Fetches the current YAML via "
			+ "getWidget first, applies only the requested change, and " + "verifies the result via screenshot.")
	PromptMessage refineWidget(
			@PromptArg(name = "widget_uid", description = "UID of the existing widget, e.g. car_charging_widget") String widgetUid,
			@PromptArg(name = "change_request", description = "What should be changed? "
					+ "E.g. 'darker background', 'larger font for the range value'") String changeRequest) {

		String body = REFINE_WIDGET_TEMPLATE.replace("{widget_uid}", nullSafe(widgetUid)).replace("{change_request}",
				nullSafe(changeRequest));

		return PromptMessage.withUserRole(new TextContent(body));
	}

	@Prompt(name = "create_page", title = "Create page", description = "Creates a new OpenHAB page with multiple widgets arranged on a canvas. "
			+ "Selects widgets based on available widget descriptions and the user's request.")
	PromptMessage createPage(
			@PromptArg(name = "description", description = "What should be visible on the page? "
					+ "E.g. 'overview of the living room: temperature, lights and blinds'") String description,
			@PromptArg(name = "page_uid", required = false, description = "Optional: desired page UID, e.g. living_room_overview") String pageUid,
			@PromptArg(name = "page_label", required = false, description = "Optional: page label shown in the sidebar") String pageLabel) {

		String body = CREATE_PAGE_TEMPLATE.replace("{description}", nullSafe(description))
				.replace("{page_uid}", nullSafe(pageUid)).replace("{page_label}", nullSafe(pageLabel));
