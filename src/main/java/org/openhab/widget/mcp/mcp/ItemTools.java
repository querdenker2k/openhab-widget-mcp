...
public class ItemTools {
	...
	public String createItem(@ToolArg(description = "The unique name of the item, e.g. MyLight") String itemName,
	...
	public String deleteItem(@ToolArg(description = "The name of the item to delete, e.g. MyOldItem") String itemName) {
		return itemService.deleteItem(itemName);
	}

	@Tool(description = "Set the stateDescription commandOptions on an OpenHAB item. "
	...
	public String setItemStateDescriptionOptions(
			...
			@ToolArg(description = "Comma-separated value=label pairs, e.g. '1=Sunset,2=Rainbow'") String options) {
		return itemService.setItemStateDescriptionOptions(itemName, options);
	}

	@Tool(description = "Set arbitrary metadata on an OpenHAB item under a given namespace. "
	...
	public String setItemMetadata(@ToolArg(description = "The item name") String itemName,
			@ToolArg(description = "Metadata namespace, e.g. stateDescription, autoupdate, expire") String namespace,
			@ToolArg(required = false, description = "The metadata value (default: ' '). Many namespaces only use config.") String value,
			@ToolArg(required = false, description = "Metadata config as JSON object string, e.g. '{\"options\":\"1=Foo,2=Bar\"}'") String configJson) {
		java.util.Map<String, Object> config = java.util.Map.of();
		if (configJson != null && !configJson.isBlank()) {
			try {
				config = new com.fasterxml.jackson.databind.ObjectMapper().readValue(configJson,
	...
	public String deleteItemMetadata(@ToolArg(description = "The item name") String itemName,
