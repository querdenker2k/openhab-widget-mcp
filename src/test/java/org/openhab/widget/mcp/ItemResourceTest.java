...
class ItemResourceTest {
    ...
    void setUp() {
        ...
        given().baseUri(url).header("Authorization", bearer)
                .contentType("application/json")
                .body("""
                        {"type":"Switch","name":"TestSwitch","label":"Test Switch","tags":[],"groupNames":[]}
                        """)
                .when().put("/rest/items/TestSwitch")
        ...
        given().baseUri(url).header("Authorization", bearer)
                ...
                .body("""
                        ...
                        """)
                .when().put("/rest/items/TestDimmer")
                .then().statusCode(anyOf(is(200), is(201)));
    ...
    void listItems_returnsAllItems() {
        given()
                ...
                .then()
                .statusCode(200)
                .body(containsString("TestSwitch"))
                .body(containsString("TestDimmer"))
                .body(containsString("OtherItem"));
    }

    @Test
    void listItems_withFilter_returnsOnlyMatchingItems() {
        given()
                .when().get("/api/items?filter=test")
                .then()
                .statusCode(200)
                .body(containsString("TestSwitch"))
                .body(containsString("TestDimmer"))
                .body(not(containsString("OtherItem")));
    }

    @Test
    void getItemState_existingItem_returnsState() {
        given()
                .when().get("/api/items/TestSwitch/state")
                .then()
                .statusCode(200);
    }
    ...
    void getItemState_unknownItem_returnsNotFoundMessage() {
        given()
                .when().get("/api/items/GhostItem/state")
                .then()
                .statusCode(200)
    ...
    void sendItemCommand_acceptsCommand() {
        given()
                ...
                .body("ON")
                .when().post("/api/items/TestSwitch/command")
                .then()
                .statusCode(200)
    ...
    void sendItemCommand_unknownItem_returnsNotFoundMessage() {
        given()
                .contentType("text/plain")
                .body("ON")
                .when().post("/api/items/GhostItem/command")
                .then()
                .statusCode(200)
