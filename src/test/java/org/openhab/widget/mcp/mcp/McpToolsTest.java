...
/**
 * Tests MCP tools in two ways: 1. Direct CDI injection — verifies business logic against a real
 * OpenHAB instance. 2. SSE transport endpoint — verifies /mcp/sse is reachable and streams events.
 */
@QuarkusTest
...
class McpToolsTest {
  ...
  @Inject WidgetTools widgetTools;

  @Inject ItemTools itemTools;
  ...
  void mcpSseEndpoint_respondsWithEventStream() throws Exception {
    ...
    HttpResponse<Stream<String>> response =
        client.sendAsync(request, HttpResponse.BodyHandlers.ofLines()).get(5, TimeUnit.SECONDS);

    assertThat(response.statusCode()).isEqualTo(200);
    assertThat(response.headers().firstValue("content-type").orElse(""))
        .contains("text/event-stream");

    CompletableFuture<String> firstData =
        CompletableFuture.supplyAsync(
            () -> response.body().filter(line -> line.startsWith("data:")).findFirst().orElse(""));
