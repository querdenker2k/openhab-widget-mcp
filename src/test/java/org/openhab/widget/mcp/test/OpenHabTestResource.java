...
public class OpenHabTestResource implements QuarkusTestResourceLifecycleManager {
  ...
  private static final GenericContainer<?> CONTAINER =
      new GenericContainer<>("openhab/openhab:5.1.4")
          ...
          .waitingFor(
              Wait.forHttp("/rest/").forStatusCode(200).withStartupTimeout(Duration.ofMinutes(5)));
  ...
  private void installInfluxPersistence() {
    ...
    try {
      ...
      HttpRequest installRequest =
          HttpRequest.newBuilder(
                  URI.create(openHabUrl + "/rest/addons/persistence-influxdb/install"))
      ...
      while (System.currentTimeMillis() < deadline) {
        HttpRequest statusRequest =
            HttpRequest.newBuilder(URI.create(openHabUrl + "/rest/addons/persistence-influxdb"))
                .GET()
                .header("Authorization", "Bearer " + accessToken)
                .build();
  ...
  private void waitForAuthEndpoint() {
    ...
    while (System.currentTimeMillis() < deadline) {
      try {
        HttpRequest request =
            HttpRequest.newBuilder(URI.create(openHabUrl + "/auth"))
                .GET()
